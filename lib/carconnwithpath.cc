...
CarConnWithPath::CarConnWithPath(const CarGrid& grid, CarPrimitiveCfg& cfg)
  :grid(grid){
SetPrimitives(1, cfg);
}
...
CarConnWithPath::CarConnWithPath(const CarGrid& grid,
...
bool CarConnWithPath::SetPrimitives(double dt, CarPrimitiveCfg& cfg){
...
bool CarConnWithPath::SetPrimitives(double dt, double vx, double kmax, int kseg, bool onlyfwd) {
  if (dt <= 0)
    return false;
...
bool CarConnWithPath::Flow(std::tuple< SE2Cell*, SE2Path, double>& pathTuple,
                           const Matrix3d& g0,
