...
bool CarTwistConnectivity::operator()(const TypedCell& from,
  ...
  for (auto& vb : vbs_) {
    ...
    Vector3d axy;
    se2_g2q(axy, g_to);
    TypedCellPtr to; to = grid_.Get(axy);
    ...
    TypedCellConnectionCostTuple path_tuple(to,vdt_final,primcost);
    paths.push_back(path_tuple);
  }
  return true;
}

template<>
bool TerrainTwistConnectivity::operator()(const TypedCell& from,
                                           std::vector<TypedCellConnectionCostTuple>& paths,
                                           bool fwd) const{
  std::set<int> cells_visited; //stored index of all the cells visited
  paths.clear();
...
bool CarPathConnectivity::
  ...
  for (auto& vb : vbs_) {
    ...
    Vector3d axy;
    se2_g2q(axy, g);
    TypedCellPtr to = grid_.Get(axy); //snap the end pose to grid
    if (!to) // "to" cell is not a part of the grid(because it has an obstacle)
      continue;
    ...
    TypedCellConnectionCostTuple path_tuple(to,path,primcost);
    paths.push_back(path_tuple);
  }
  return true;
}
